...
def getMaterials(matsId):
    ...
    for materialsGroups in ui.element(matsId).childs(): # for each group in materials folder
        ...
        #             matAbsorp.insert(0, matName)
        #             materials.append(matAbsorp)
        if materialsGroups[1]==ui.element_type.ELEMENT_TYPE_SCENE_BDD_MATERIAUX_USER: # if group is generated by the user
            for materialGroup in ui.element(materialsGroups[0]).childs(): # for element in group
...
def createMaterialSave(surfaceMatDict, areaDict):
    saveData=[['Surface Name','Material Used','Surface Area']]
    for surface, material in surfaceMatDict.items():
        area=areaDict[surface]
        join=[surface, material, area]
...
class manager:
    ...
    def runCalculation(self,elementId):
        punctualId=elementId # id of the punctual rec folder
